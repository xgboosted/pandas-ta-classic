...
- **yfinance / alpha-vantage**: For downloading OHLCV data — install directly (``pip install yfinance alpha-vantage``). The built-in ``df.ta.ticker()`` / ``ta.yf()`` / ``ta.av()`` helpers were removed in 0.6.53; call yfinance / alpha-vantage directly and pass the DataFrame in (see ``examples/fetch_market_data.py``)

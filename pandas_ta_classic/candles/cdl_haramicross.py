...
def _detect(ca, out, **kwargs):
    ...
    for i in range(start_idx, len(out)):
        ...
        ) and ca.real_body[i] <= ca.candle_average(
            CandleSetting.BodyDoji, body_doji_total, i
        ):
            if max(ca.close[i], ca.open[i]) < max(
                ca.close[i - 1], ca.open[i - 1]
            ) and min(ca.close[i], ca.open[i]) > min(ca.close[i - 1], ca.open[i - 1]):
                out[i] = -ca.color[i - 1] * 100
            elif max(ca.close[i], ca.open[i]) <= max(
                ca.close[i - 1], ca.open[i - 1]
            ) and min(ca.close[i], ca.open[i]) >= min(ca.close[i - 1], ca.open[i - 1]):

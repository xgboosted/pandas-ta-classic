...
from pandas_ta_classic.utils._numba import NUMBA_AVAILABLE, jma_numba_core


def jma(close, length=None, phase=None, offset=None, use_numba=True, **kwargs):
    ...
    if use_numba and NUMBA_AVAILABLE:
        result_values = jma_numba_core(close.values, _length, phase)
        # Remove initial lookback data
        result_values[0 : _length - 1] = npNaN
        jma = Series(result_values, index=close.index)
    else:
        # Fall back to pure Python implementation
        # Define base variables
        jma_result = npZeroslike(close)
        volty = npZeroslike(close)
        v_sum = npZeroslike(close)

        kv = det0 = det1 = ma2 = 0.0
        jma_result[0] = ma1 = uBand = lBand = close[0]

        # Static variables
        sum_length = 10
        _len = 0.5 * (_length - 1)
        pr = 0.5 if phase < -100 else 2.5 if phase > 100 else 1.5 + phase * 0.01
        length1 = max((npLog(npSqrt(_len)) / npLog(2.0)) + 2.0, 0)
        pow1 = max(length1 - 2.0, 0.5)
        length2 = length1 * npSqrt(_len)
        ...
        for i in range(1, m):
            ...
            det1 = ((ma2 - jma_result[i - 1]) * (1 - alpha) * (1 - alpha)) + (
                alpha * alpha * det1
            )
            jma_result[i] = jma_result[i - 1] + det1

        # Remove initial lookback data and convert to pandas frame
        jma_result[0 : _length - 1] = npNaN
        jma = Series(jma_result, index=close.index)

...
- **pre-commit runs the same ruff gate as CI.** The `ruff-check` hook reads `[tool.ruff.lint]` with no `--select` and no package-only `files:` filter. Black stays package-only in pre-commit, matching CI's `black --check pandas_ta_classic/`.
...
- **mypy** — `make typecheck` (targets Python 3.12: numpy >= 2.5 stubs cannot be parsed below it); blocking in CI.

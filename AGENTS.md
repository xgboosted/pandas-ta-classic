...
- **Generated `core.pyi`** is normalized with `ruff check --fix` before `black` (CI does gen → ruff --fix → black → `git diff --exit-code`), so the stub matches lint-fixed, PEP-604 source. If you change `tools/gen_core_stub.py`, keep that pipeline.

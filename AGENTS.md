...
- **Testing:** pytest (primary, matches CI). Run `pytest tests/ -v` for full suite (oracle deps required; see Validation section), or the smallest relevant test module for the changed area (e.g., `pytest tests/test_indicator_momentum.py -v`). Hypothesis property-based tests also use pytest. `filterwarnings` in `pyproject.toml` turns any `UserWarning`, `DeprecationWarning` or `FutureWarning` raised from `pandas_ta_classic` into a failure: a deprecated pandas/numpy call is fixed, not silenced.
...
- **PR reviews:** A review is done only when its result is posted as a comment on the PR (GitHub MCP server, `add_issue_comment`); a verdict that exists only in a chat or a local report is not a review. Write the comment for the PR author and other maintainers: the verdict (approved / changes needed), what was checked and the results (test suite and gate commands, CI, a merge test against current `main`), any commit pushed to the branch during the review and what it changed, and the open points with who owns them. Post it before the PR is merged.
...
- Version auto-generated by setuptools-scm in `pandas_ta_classic/_version.py` (gitignored). GitHub source archives read it from `.git_archival.txt` (`export-subst`); there is no `fallback_version`, so a build without git metadata fails instead of reporting `0.0.0`
...
11. **Deprecate in a released version before removing.** A warning that has not shipped in a tagged release is not a deprecation. Documentation names the version that shipped it (`.. deprecated:: 0.8.32`), never a guessed next version. Announce removals with a plain `.. note::`: `.. versionremoved::` needs Sphinx ≥ 7.3 and the docs floor is lower. **Exception: a parameter that never had an effect** (accepted but never read, so removing it changes no result) is removed in the next breaking release without a deprecation step. Removal must reject it: an indicator's `**kwargs` would otherwise swallow the old name silently, so raise `TypeError` naming the parameter (`cpr(interval=...)`). Mark the CHANGELOG entry **BREAKING** under Removed and leave a removal note where the parameter was documented. A removed name that is also a strategy-wide argument (`drift`, `length`) is absorbed with a comment under rule 3 instead, so `df.ta.strategy(..., drift=N)` keeps working (`cfo`, `kst`, `rsx`, ...). **0.9.0 ships no deprecation at all** (maintainer decision): anything renamed or removed in it raises `TypeError` at once instead of warning, including names introduced in the same release (`max_drawdown(all=)`, `df.ta(version=)`).
12. **Follow the removal plan the warning promised.** If a warning says "the default changes in the next breaking release", that release makes the change (`ichimoku` → DataFrame in 0.9.0). The old form warns until its announced removal, unless the release drops it outright (0.9.0 removed `as_dataframe` and the tuple).

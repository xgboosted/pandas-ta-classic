...
* **`jma` ~130× faster (numba)**: the Jurik Moving Average per-bar recursion (volatility bands + 3-stage filter) moved into a numba `@njit` loop with scalar coefficients precomputed by the caller (`~50 ms` → `~0.4 ms` per 5000-bar call). Output is numerically equivalent but not bit-for-bit identical: the trailing-window average uses `np.mean` in place of the original `np.average`, whose internal reduction order differs at the ULP level (max relative deviation ~4e-16, i.e. full float64 precision) even for these small windows; the difference compounds through the recursive `jma[i] = jma[i-1] + det1` update but is inconsequential for any practical use. Falls back to a plain Python loop without numba (same `np.mean`-based deviation applies there too).

...
### Fixed
* **Type-check cleanup (mypy)**: Resolved all real type findings and eliminated stale suppressions. `is_percent()` now returns `TypeGuard[Union[int, float]]` so numeric guards narrow correctly; `_meta.py` aliases the `importlib.metadata.version` import to avoid a name collision; `xsignals()` calls `tsignals()` with explicit keyword arguments; `cdl_pattern`/`skew`/`kurtosis` gained honest annotations, and `ebsw`/`chop` were minorly restructured (float init / expression order) — all runtime-equivalent. Removed five per-module `mypy` override blocks (two stale, three replaced by the fixes above); `talib` is now scoped to `follow_imports = "skip"` because its Function API is polymorphic (Series in → Series out) while its stubs declare a fixed `ndarray` return. `mypy` is clean at the Python 3.10 floor.

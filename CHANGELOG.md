...
* **mypy type-checking is now gated in CI**: the `code-quality` job runs `mypy` against the `requires-python` floor (the config and `make typecheck` target already existed; nothing enforced them, so type errors could regress silently). Passes clean on the current tree.

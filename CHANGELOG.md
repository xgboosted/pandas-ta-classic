...
* **`jma` ~130× faster (numba)**: the Jurik Moving Average per-bar recursion (volatility bands + 3-stage filter) moved into a numba `@njit` loop with scalar coefficients precomputed by the caller (`~50 ms` → `~0.4 ms` per 5000-bar call). Output is bit-for-bit identical across phases; falls back to a plain Python loop without numba.
...
* **Candlestick patterns ~50–100× faster (numba)**: every native `cdl_*` pattern's per-bar detection loop (the TA-Lib incremental running-window algorithm) moved into a numba `@njit` kernel over numpy arrays, with the average-factor constants and range arrays hoisted by the caller. `cdl_pattern(name="all")` drops from `~290 ms` to single-digit ms per 5000-bar call. Output is bit-for-bit identical for every pattern; falls back to a plain Python loop without numba.

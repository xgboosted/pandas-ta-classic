...
* **Import conventions are now enforced in CI, not just followed**: the `code-quality` job gained a blocking `ruff check .` over the whole repo. The pre-existing ruff step passes `--select E9,F63,F7,F82`, which *overrides* the config's rule set — so star imports (`F403`), imports after code (`E402`) and ambiguous names (`E741`) were previously not gated anywhere despite being clean. `[tool.ruff.lint]` now also enables `ICN` with `banned-from = ["numpy"]`: ruff's conventional import aliases (`numpy as np`, `pandas as pd`, …) are enforced repo-wide, and `from numpy import x` is rejected in every form including the `as npX` aliases. `from numpy.lib.stride_tricks import …` is unaffected, and pandas is deliberately not banned — `from pandas import Series` is the established style at 300+ sites. Two further conventions ruff cannot express — no `sys.path` bootstrapping outside `custom.py`, and no deferred `numpy`/`pandas` imports inside function bodies — are documented in AGENTS.md and enforced by review (ruff's `PLC0415` is unusable for the latter because it flags the ~90 intentional optional-dependency deferrals too).

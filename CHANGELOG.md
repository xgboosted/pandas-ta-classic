...
* **mypy type-checking is now gated in CI**: the `code-quality` job runs `mypy` at Python 3.12 (the config and `make typecheck` target already existed; nothing enforced them, so type errors could regress silently). Targets 3.12 rather than the 3.10 `requires-python` floor because numpy 2.x ships PEP 695 `type` statements in its stubs that mypy cannot parse below 3.12; runtime support for the floor is covered by the test matrix. `talib` (optional, absent in the lint env) is marked `ignore_missing_imports`. Passes clean on the current tree.
* **Adopted ruff's full default rule set (0.16) and modernized the codebase to match**: the lint pin floor is now `ruff>=0.16.0` (and `.pre-commit-config.yaml` tracks `v0.16.0`) so every environment runs the expanded default, not an older narrow one. `ruff check .` now runs ruff's expanded default (pyupgrade `UP`, isort `I`, flake8-bugbear `B`, `SIM`, `PL`, `RUF`, …) plus `ICN`, and the repo was auto-fixed to pass it — most visibly PEP 604 annotations (`Optional[X]` → `X | None`, 2100+ sites; valid on the 3.10+ floor) and sorted imports. Library code (`pandas_ta_classic/`) carries no per-file ignores; its three intentional broad catches / accessor-contract raises use inline `# noqa` with a reason. `[tool.ruff.lint.per-file-ignores]` scopes a few script-oriented rules out of non-shipped code (notebook `#!pip` cells, demo idioms in `examples/`; best-effort swallows in `tools/`/`tests/`). The generated `core.pyi` is normalized through `ruff --fix` before formatting so it stays in sync. **Note:** this re-enables import-sorting (isort rule `I001`) that the earlier isort removal had dropped — now handled by ruff, not a separate tool.
...
* **Numpy import style enforced repo-wide**: the remaining aliased and bare `from numpy import …` imports now use `import numpy as np` + `np.x` (`ui`, `stdev`, `log_return`, `drawdown`, `lrsi`, `utils/_core.py`, `examples/watchlist.py`). Deferred `numpy`/`pandas` imports inside function bodies were hoisted to module scope (`utils/_core.py`, `utils/_math.py`, `adosc`, `sarext`, `linreg`) — both are hard dependencies, so deferring them bought nothing. Optional-dependency imports (`talib`, `tulipy`, `tqdm`) stay deferred by design. Internal only; no public API change.
* **Import conventions are now enforced in CI, not just followed**: the `code-quality` job's blocking ruff step is a single repo-wide `ruff check .`. Previously it passed `--select E9,F63,F7,F82`, which *replaces* the configured rule set — so star imports (`F403`), imports after code (`E402`) and ambiguous names (`E741`) were not gated anywhere despite being clean. The rule set now lives as an explicit `select` in `[tool.ruff.lint]` (not `extend-select`, so it doesn't ride on ruff's default, which expanded in 0.16 — this keeps the gate stable as the `>=` ruff pin floats to newer releases). `[tool.ruff.lint]` now also enables `ICN` with `banned-from = ["numpy"]`: ruff's conventional import aliases (`numpy as np`, `pandas as pd`, …) are enforced repo-wide, and `from numpy import x` is rejected in every form including the `as npX` aliases. `from numpy.lib.stride_tricks import …` is unaffected, and pandas is deliberately not banned — `from pandas import Series` is the established style at 300+ sites. Two further conventions ruff cannot express — no `sys.path` bootstrapping outside `custom.py`, and no deferred `numpy`/`pandas` imports inside function bodies — are documented in AGENTS.md and enforced by review (ruff's `PLC0415` is unusable for the latter because it flags the ~90 intentional optional-dependency deferrals too).
* **Remaining `sys.path` bootstrap removed** from `tests/__init__.py`; `__import__("pathlib")` inlines in `tests/test_oracle_talib.py` and `tests/test_native_indicators.py` replaced with a top-level `from pathlib import Path`. The `sys.path.append` in `custom.py` is functional (it loads user indicator directories) and is unaffected.
...
### Removed
* **Example `Watchlist` data-fetcher** (`examples/watchlist.py`): the class downloaded market data via yfinance / Alpha Vantage — the same built-in fetching removed from the library. Trimmed `watchlist.py` to just its `colors()` charting-palette helper. The three example notebooks that used it (`AIExample`, `example`, `PandasTA_Strategy_Examples`) now fetch OHLCV with yfinance directly and apply strategies via `df.ta.strategy(...)`; the strategy notebook was rewritten end-to-end and executes clean. Dead (imported-never-called) `alphaVantageAPI` imports removed from the notebooks.
* **Built-in data fetching — `df.ta.ticker()`, `ta.yf()`, `ta.av()`** (and the `pandas_ta_classic/utils/data/` package): removed. Data fetching is out of scope for a technical-analysis library. Fetch OHLCV with `yfinance` / `alpha-vantage` directly and pass the DataFrame to pandas-ta-classic; see `examples/fetch_market_data.py` for the replacement patterns. The `data` optional extra, the `alpha-vantage`/`yfinance` `Imports` probes, and the corresponding mypy overrides are gone; `integration` now aliases `backtest` only. **Breaking:** callers of these three symbols must switch to fetching data themselves.
...
* **`AVG_PERIOD`** (`candles/_cdl_math.py`): its only consumer, `CandleArrays.candle_average()`, was removed in the dead-code audit; the constant was left behind.
...
### Fixed
* **`combination()` silently ignored `multichoose`**: the kwarg was an alias for `repetition` before it was dropped in the `math.comb` rewrite, so `combination(n=10, r=4, multichoose=True)` returned 210 instead of 715 — a wrong number, not an error. The alias is restored, and any unrecognised keyword now raises `TypeError` instead of being discarded (a typo such as `reptition=True` was previously silent). `pascals_triangle()` passes only `n`/`r`, so `pwma` output is unchanged.
* **`asc=False` was a no-op on `wma`, `fwma`, `pwma`, `swma`**: the `asc = asc if asc else True` guard forced `True` (`False` is falsy), so the documented "recent values weigh more" toggle silently returned the ascending result. `wma` carried an unreachable weight-reversal branch; the other three never referenced `asc` at all. Descending weights were never implemented in any of the four, so passing `asc=False` now raises `ValueError` rather than returning a value the caller did not ask for; `asc=True` and the default are unchanged. The dead branch in `wma`, and the `asc=False` example in `ma()`'s docstring, are removed.
* **`df.ta.ticker(ds=...)` silently discarded the data-source argument**: `ds` was popped and ignored, so `ds="alphavantage"` returned Yahoo Finance data with no signal. Passing it now emits a `FutureWarning`. (The method as a whole is already deprecated.)

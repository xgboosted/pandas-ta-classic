...
* **TA-Lib passthrough for the Hilbert Transform family**: `ht_dcperiod`, `ht_dcphase`, `ht_phasor`, `ht_sine`, `ht_trendmode`, and `ht_trendline` now accept `talib=True` to dispatch to their TA-Lib counterparts (`HT_DCPERIOD`, `HT_DCPHASE`, `HT_PHASOR`, `HT_SINE`, `HT_TRENDMODE`, `HT_TRENDLINE`). The native Python Hilbert loop is ~73–76 ms per 5000-bar call; the TA-Lib path is ~0.2–1.5 ms (roughly 50–400× faster) with identical output (max diff ≤1e-10).
